...
/**
 * Simple test runner to execute comprehensive NLP tests and save results
 */
@SpringBootTest
public class TestRunner {
    
    @Autowired
    private MockNLPEngine nlpEngine;
    
    @Test
    public void runComprehensiveTest() {
        System.out.println("=== Starting Comprehensive NLP Test ===");
        ...
        for (int i = 0; i < allTestCases.length; i++) {
            ...
            try {
                ...
                String queryType = getQueryType(response);
                String actionType = getActionType(response);
        ...
        results.append("Processing Time: ").append(LocalDateTime.now()).append("\n");
        ...
        // Save to file
        try {
            String fileName = "nlp_test_results_" + LocalDateTime.now().toString().replaceAll(":", "-") + ".txt";
    ...
    // Helper methods to extract information from NLP response
    ...
    private String getAccountNumber(NLPResponse response) {
    ...
    private String getQueryType(NLPResponse response) {
        return response.getQueryMetadata() != null ? response.getQueryMetadata().getQueryType() : "UNKNOWN";
    }
    
    private String getActionType(NLPResponse response) {
        return response.getQueryMetadata() != null ? response.getQueryMetadata().getActionType() : "unknown_action";
    }
    ...
    private boolean isComplexQuery(String input) {
        return input.contains("created by") || input.contains("vinod") || input.contains("mary") || 
               ...
               input.contains("between") || input.contains("after");
    }
