...
public class MockNLPEngine {
    ...
    private String normalizeInput(String input) {
        ...
        normalized = normalized.replace("合同", "contract");
        normalized = normalized.replace("契約", "contract");
        normalized = normalized.replace("部品", "part");
    ...
    private NLPResponse generateDomainResponse(String originalInput, String correctedInput, 
        ...
        String actionType = generateActionType(domain, originalInput, entities);
        ...
        switch (domain) {
            case CONTRACTS:
                if (entities.containsKey("contractNumber")) {
                    ...
                    return "contracts_by_creator";
                } else if (entities.containsKey("accountNumber")) {
                    return "contracts_by_account";
                } else if (lowerInput.contains("expired")) {
                    return "contracts_expired";
                } else {
                    return "contracts_general_query";
                }
                
            case PARTS:
                if (lowerInput.contains("failed") || lowerInput.contains("error")) {
                    return "parts_failure_analysis";
                } else if (lowerInput.contains("validation")) {
                    return "parts_validation_check";
                } else if (entities.containsKey("partNumber")) {
                    return "parts_by_partNumber";
                } else {
                    return "parts_general_query";
                }
    ...
    public Set<DomainType> getAvailableDomains() {
        return Set.of(DomainType.CONTRACTS, DomainType.PARTS, DomainType.HELP);
    }
